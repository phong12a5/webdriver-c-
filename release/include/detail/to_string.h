#ifndef FDRIVER_TO_STRING_H
#define FDRIVER_TO_STRING_H
...
namespace fdriver_to_string_impl {

template<typename T>
std::ostream& operator << (std::ostream& stream, const T& value) {
    fdriver::detail::to_string_impl::WriteNonStreamableValue(value, stream);
	return stream;
}
...
void ToStreamImpl(const T& value, std::ostream& stream, DefaultTag) {
    using namespace fdriver_to_string_impl;
	stream << value;
}
