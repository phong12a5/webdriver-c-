#ifndef FDRIVER_DETAIL_RESOURCE_H
#define FDRIVER_DETAIL_RESOURCE_H
...
private:
	...
	picojson::value ProcessResponse(
...
#ifdef DEBUG_MODE
        std::cout << "response: " << (response.is<picojson::null>()? "null" : response.serialize()) << std::endl;
#endif

		FDRIVER_CHECK(error_message.empty(),
			Fmt() << "JSON parser error (" << error_message << ")"
			);

		FDRIVER_CHECK(response.is<picojson::object>(), "Server response is not an object");

        response_status_code::Value status = response_status_code::Value::kUnknownError;
        ...
        FDRIVER_CHECK(response.contains("value"), "Server response has no member \"value\"");
		const auto& value = response.get("value");

		if (http_response.http_code == 500) { // Internal server error
			FDRIVER_CHECK(value.is<picojson::object>(), "Server returned HTTP code 500 and \"response.value\" is not an object");
			FDRIVER_CHECK(value.contains("message"), "Server response has no member \"value.message\"");
			FDRIVER_CHECK(value.get("message").is<std::string>(), "\"value.message\" is not a string");
			FDRIVER_THROW(Fmt() << "Server failed to execute command ("
        ...
        if (response.contains("status")) {
                    FDRIVER_CHECK(status == response_status_code::kSuccess, "Non-zero response status code");
                }
        FDRIVER_CHECK(http_response.http_code == 200, "Unsupported HTTP code");
