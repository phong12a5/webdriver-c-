#ifndef FDRIVER_SESSION_H
#define FDRIVER_SESSION_H
...
class WEBDRIVER_EXPORT Session { // copyable
public:
    ...
    WEBDRIVER_EXPORT Element FindElement(const By& by) const;
    WEBDRIVER_EXPORT std::vector<Element> FindElements(const By& by) const;
    ...
    void DeleteSession() const; // No need to delete sessions created by FDriver or Client
	virtual ~Session() {}
...
private:
	...
	template<typename T>
    void InternalEval(const std::string& fdriver_command,
